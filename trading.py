...
async def perform_trade(market):
    ...
    async with market_locks[market]:
        try:
            ...
            # ---- Strategy engine path (opt-in per param-type via the sheet) ----
            # When 'use_strategy_engine' is set, hand off to the quantitative
            # quoting brain. The legacy logic below is left fully intact and runs
            # whenever the flag is absent, so enabling the engine is reversible and
            # never silently changes a live bot's behaviour.
...
def _reconcile_engine_orders(client, token, neg_risk, decision, existing, tick_size, min_price, max_price):
...
async def run_engine_outcomes(market, row, params, round_length):
    ...
    Reuses the legacy position-merging and order-book plumbing, then delegates the
    ...
    for detail in deets_meta:
        ...
        print(f"[engine] {detail['answer']}: fair={decision.fair_value:.4f} "
              ...
              f"bid={decision.bid_price} ask={decision.ask_price} reasons={decision.reasons}")

        # Risk-free Dutch book takes priority if one is detected (currently dormant
        # until the opposite token's book is subscribed; see strategy_adapter).
